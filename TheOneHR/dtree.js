...
function selectRight(obj)
{
	...
	for(var i=0;i<rights.length;i++)
	{
		...
		if(rights[i].value.indexOf(obj.value)!=-1)
		{
			rights[i].checked=obj.checked;
		}
		if(obj.value.indexOf(rights[i].value)!=-1&&obj.value!=rights[i].value)
		{	//alert(11);
			rights[i].checked=obj.checked;
			parentNode = rights[i];
	...
	{
		//alert(parentNode.value);
		for(var i=0;i<rights.length;i++)
		{
			if(rights[i].value.indexOf(parentNode.value)!=-1&&rights[i].checked)
				selectFlag = true;
		}
	}
	//alert(selectFlag);
	if(!selectFlag)
	{
		parentNode.checked=obj.checked;
	}	
	else{
...
dTree.prototype.node = function (node, nodeId) {
	var str = "<div id=\"div" +node.id + "\" class=\"dTreeNode\" oncontextmenu=\"contextMenu('"+node.id+"');return false;\">" + this.indent(node, nodeId);
	if (this.config.useIcons) {
		...
		if(node.flag==1)
		{
			str += "<img id=\"i" + this.obj + nodeId + "\" src=\"" + ((node._io) ? node.iconOpen : node.icon) + "\" alt=\"\" /><input type='checkbox'name='right' checked value=\"" + node.id + "\" onclick='selectRight(this)'/>";
		}
		if(node.flag==2)
		{
			str += "<img id=\"i" + this.obj + nodeId + "\" src=\"" + ((node._io) ? node.iconOpen : node.icon) + "\" alt=\"\" /><input type='checkbox' name='right' value=\"" + node.id + "\" onclick='selectRight(this)'/>";
		}
	}
	if (node.url) {
